              ...
              <div style={s.admissionFormRow}>
               <div style={s.admissionFormGroup}>
                  ...
                  <select style={s.admissionInput} name="gender" value={form.gender} onChange={handle} required>
                    <option value="" style={s.admissionOption}>Select Gender</option>
                    <option style={s.admissionOption}>Male</option>
                    <option style={s.admissionOption}>Female</option>
                  </select>
                </div>
                <div style={s.admissionFormGroup}>
                  ...
                  <select style={s.admissionInput} name="program" value={form.program} onChange={handle} required>
                    <option value="" style={s.admissionOption}>Select Program</option>
                    <option style={s.admissionOption}>Play Group (1.5-2.5 yrs)</option>
                    <option style={s.admissionOption}>Nursery (2.5-3.5 yrs)</option>
                    <option style={s.admissionOption}>LKG (3.5-4.5 yrs)</option>
                    <option style={s.admissionOption}>UKG (4.5-5.5 yrs)</option>
                  </select>
                </div>
              </div>
              ...
              <div style={s.admissionFormRow}>
                ...
                <div style={s.admissionFormGroup}>
                  ...
                  <select style={s.admissionInput} name="relation" value={form.relation} onChange={handle} required>
                    <option value="" style={s.admissionOption}>Select Relation</option>
                    <option style={s.admissionOption}>Father</option>
                    <option style={s.admissionOption}>Mother</option>
                    <option style={s.admissionOption}>Guardian</option>
                  </select>
                </div>
              </div>
              <div style={s.admissionFormRow}>
          ...
          <div style={s.panel}>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={s.tbl}>
                ...
                <tbody>
                 {fAdmissions.map((a, i) => (
                    <tr key={i} style={{ ...s.tr, cursor: 'pointer' }} onClick={() => setViewItem({ type: 'admission', data: a })}>
                      ...
                      <td style={{ ...s.td, whiteSpace: 'nowrap' }}>
                        {/* Notice the e.stopPropagation() here to prevent opening the modal when deleting! */}
          ...
          <div style={s.panel}>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={s.tbl}>
                ...
                <tbody>
                {fEnquiries.map((e, i) => (
                    <tr key={i} style={{ ...s.tr, cursor: 'pointer' }} onClick={() => setViewItem({ type: 'enquiry', data: e })}>
                      <td style={s.td}>{i + 1}</td>
    ...
    <div style={s.loadingScreen}>
      <style>{`
        ...
        .loading-sun-img {
          ...
          animation: spinSun 8s linear infinite; object-fit: contain;
        }
        .loading-dots { display: flex; justify-content: center; gap: 8px; margin-top: 24px; }
        .loading-dots span { width: 10px; height: 10px; background: #FF6B35; border-radius: 50%; animation: bounce 1s infinite; }
        .loading-dots span:nth-child(2) { animation-delay: 0.2s; background: #4ECDC4; }
        .loading-dots span:nth-child(3) { animation-delay: 0.4s; background: #FFD93D; }
      `}</style>
      <div style={s.loadingLogo}>
        <img src="/assets/loading-sun.png" alt="Loading" className="loading-sun-img" />
        <h1 style={{ fontFamily: 'sans-serif', fontSize: 36, color: '#FFD93D', margin: '0 0 4px' }}>VT Kindergarten</h1>
        <p style={{ color: '#4ECDC4', fontSize: 18, margin: 0 }}>Admin Portal</p>
...
const s = {
  loadingScreen: { position: 'fixed', inset: 0, background: '#1a1a2e', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 99999, animation: 'fadeOutLoading 0.5s 2s forwards' },
  loadingLogo: { textAlign: 'center', color: 'white' },
  ...
  loginLogo: { width: 160, height: 160, display: 'block', margin: '0 auto 16px', padding: 0, borderRadius: '50%', background: 'transparent', objectFit: 'cover', transform: 'scale(1.12)', clipPath: 'circle(50%)', boxShadow: '0 12px 32px rgba(255,107,53,0.24)', boxSizing: 'border-box' },
  fg: { marginBottom: 20 },
 ...
 sidebar: { boxSizing: 'border-box', width: 260, background: 'linear-gradient(180deg,#1a1a2e,#16213e)', padding: '24px 16px', display: 'flex', flexDirection: 'column', gap: 8, position: 'fixed', top: 0, left: 0, bottom: 0, overflowY: 'auto', zIndex: 100 },
  ...
  sideLogo: { width: 64, height: 64, display: 'block', padding: 0, borderRadius: '50%', background: 'transparent', objectFit: 'cover', transform: 'scale(1.12)', clipPath: 'circle(50%)', boxShadow: '0 8px 22px rgba(255,107,53,0.22)', flexShrink: 0, boxSizing: 'border-box' },
  ...
  admissionOption: { background: '#1a1a2e', color: '#ffffff' },
